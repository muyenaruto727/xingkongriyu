...
const Introduction = () => {
  ...
  const generateGameCards = (row) => {
    ...
    if (currentRow === '全部') {
      // 选择全部行
      hiragana.forEach((hiraganaRow, rowIndex) => {
        hiraganaRow.characters.forEach((hiraganaChar, charIndex) => {
          const katakanaChar = katakana[rowIndex].characters[charIndex].char;
          kanaPairs.push({
            hiragana: hiraganaChar.char,
            katakana: katakanaChar
          });
        });
      });
    } else {
      // 选择特定行
      const rowIndex = hiragana.findIndex(r => r.row === currentRow);
      if (rowIndex !== -1) {
        hiragana[rowIndex].characters.forEach((hiraganaChar, charIndex) => {
          const katakanaChar = katakana[rowIndex].characters[charIndex].char;
          kanaPairs.push({
            hiragana: hiraganaChar.char,
            katakana: katakanaChar
    ...
    // 确定卡片数量和布局
    let cardCount;
    if (['あ行', 'か行', 'さ行', 'た行', 'な行', 'は行', 'ま行', 'ら行', 'や行', 'わ行'].includes(currentRow)) {
      // 这些行使用5x2布局，每次抽取5个平假名+5个片假名
      cardCount = 5;
    } else {
      // 全部使用6x6布局
      cardCount = 18;
    ...
    for (let i = 0; i < cardCount; i++) {
      if (i < kanaPairs.length) {
        const kanaPair = kanaPairs[i];
  ...
  const startGame = (row) => {
    ...
    const cards = generateGameCards(rowToUse);
    ...
    // 使用setTimeout确保gameCards更新后再设置gameStarted
    setTimeout(() => {
      setGameStarted(true);
    }, 0);
  };
  ...
  const handleCardClick = (card) => {
    // 如果卡片已经匹配或已经被选择，不做处理
    if (card.matched || selectedCards.includes(card.id)) {
      return;
    }
        ...
        <section className="pt-12 pb-12 md:pt-20 md:pb-20">
          <div className="container">
            ...
            <div className="bg-white rounded-xl shadow-lg p-8 border border-gray-100">
              ...
              {activeTab === 'game' && (
                <div>
                  <h3 className="text-2xl font-semibold mb-6 text-gray-800">五十音消消乐</h3>
                  <p className="text-gray-600 mb-6 leading-relaxed">
                    点击相同的假名进行消除，锻炼你的假名识别能力！
                  </p>
                  
                  <div className="mb-6">
                    <div className="flex flex-wrap gap-2 mb-4">
                      {['全部', 'あ行', 'か行', 'さ行', 'た行', 'な行', 'は行', 'ま行', 'や行', 'ら行', 'わ行'].map(row => (
                        <button
                          key={row}
                          className={`px-3 py-1 rounded-full text-sm ${selectedRow === row ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
                          onClick={() => {
                            setSelectedRow(row);
                            startGame(row);
                          }}
                  ...
                  {!gameStarted || gameCards.length === 0 ? (
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-6">
                        <div className="text-xl font-semibold">分数: {score}</div>
                        <div className="text-xl font-semibold">用时: {currentTime}秒</div>
                        <button 
                      ...
                      <div className={`grid ${['あ行', 'か行', 'さ行', 'た行', 'な行', 'は行', 'ま行', 'ら行', 'や行', 'わ行'].includes(selectedRow) ? 'grid-cols-5 gap-3' : 'grid-cols-6 gap-3'} mb-6`}>
                        {gameCards.map(card => {
                        ...
                        {/* 对于や行和わ行，添加空白占位符以保持5x2布局 */}
                        {['や行', 'わ行'].includes(selectedRow) && gameCards.length < 10 && Array.from({ length: 10 - gameCards.length }).map((_, index) => (
                          <div key={`empty-${index}`} className="aspect-square bg-gray-50 rounded-lg border border-gray-100"></div>
                        ))}
                      </div>
                      
                      {gameOver && (
                        <div className="bg-green-100 p-6 rounded-lg text-center">
                          <h4 className="text-xl font-semibold mb-2">游戏结束！</h4>
