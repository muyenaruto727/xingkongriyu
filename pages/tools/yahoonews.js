          ...
          <div className="tool-container-wide">
            ...
            <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_340px] lg:items-end">
              ...
              <aside className="tool-panel p-6">
                <p className="text-sm font-semibold text-slate-500">新闻面板</p>
                ...
                <dl className="mt-5 space-y-3 text-sm">
                  ...
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <dd className="font-semibold text-slate-800">Yahoo Japan</dd>
                  </div>
                  <div className="flex items-center justify-between gap-4">
            ...
            <div className="tool-panel mt-10 mb-6 flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">点击新闻会在新窗口打开原文。</p>
              </div>
              <button
                type="button"
            ...
            {loading && (
              <div className="grid gap-4">
                {Array.from({ length: 6 }).map((_, i) => (
                  <div key={i} className="tool-panel p-5">
            ...
            {!loading && newsList.length === 0 && (
              <div className="tool-panel px-6 py-16 text-center">
                ...
                <p className="mb-6 text-slate-500">点击「刷新新闻」从 Yahoo Japan 获取最新新闻。</p>
                <button
                  type="button"
                  onClick={handleRefresh}
                  disabled={refreshing}
              ...
              <div className="grid gap-4">
                {newsList.map((news, index) => (
                  <a
                    ...
                    className="tool-news-card group"
                  >
                    <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                      <div className="min-w-0">
                        <div className="mb-3 flex flex-wrap items-center gap-3 text-xs font-semibold text-slate-500">
                          <span className="tool-chip px-2.5 py-1 text-slate-600">#{String(index + 1).padStart(2, '0')}</span>
                        ...
                        {news.description && (
                          <p className="mt-3 line-clamp-4 text-sm leading-7 text-slate-600 md:text-base">
                            {news.description}
                          </p>
                        )}
                      </div>
                      <span className="hidden md:flex tool-icon-button">
