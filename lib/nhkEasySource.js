...
function parseNhkEasyHtml(html) {
  ...
  nhkeasy({ separator: ' ' }, html, (err, data) => {
    parseError = err;
    parsed = data;
  });

  if (parseError) {
    throw new Error(typeof parseError === 'string' ? parseError : parseError.message);
  }

  return {
    title: stripHtml(parsed?.title || ''),
    description: stripHtml(parsed?.article || '').substring(0, 500),
  };
}
