...
function isApiRequestError(error) {
  return error instanceof ApiRequestError || Boolean(error?.isApiRequestError);
}

function handleCaughtError(context, error) {
  if (isApiRequestError(error)) {
    return;
  }

  const contextMessage = typeof context === 'string' ? context.replace(/[:：]\s*$/, '') : '';
  const userMessage = contextMessage && hasUserReadableText(contextMessage) && !isTechnicalErrorMessage(contextMessage)
    ? contextMessage
...
export const api = {
  ...
  getInvitationCodeList: async (params = {}) => {
    return request(withQuery('/api/invitation-codes', params));
  },

  createInvitationCode: async (data) => {
    return request('/api/invitation-codes', {
      method: 'POST',
      body: JSON.stringify(data),
    });
  },
  ...
  createOneOnOneBooking: async (data) => {
    return request('/api/one-on-one-bookings', {
      method: 'POST',
      body: JSON.stringify(data),
    });
  },

  getOneOnOneBookings: async (params = {}) => {
    return request(withQuery('/api/one-on-one-bookings', params));
  },

  updateOneOnOneBooking: async (data) => {
    return request('/api/one-on-one-bookings', {
      method: 'PUT',
      body: JSON.stringify(data),
    });
  ...
  // Yahoo Japan News
  getYahooNews: async (params = {}) => {
    return request(withQuery('/api/yahoo-news', params));
  },

  refreshYahooNews: async () => {
    return request('/api/yahoo-news', {
      method: 'POST',
    });
  },
};
